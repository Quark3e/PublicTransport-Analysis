...
GUINC::lazyimgui::lazyimgui(bool _init, size_t _width, size_t _height) {
    if(__init) throw std::runtime_error("ERROR: lazyimgui(bool) this library instance is already initialised.");
    
    GUINC::win_dim[0] = _width;
    GUINC::win_dim[1] = _height;

    if(_init) this->init();
}
GUINC::lazyimgui::~lazyimgui() {
    if(!__init) return;
...
bool GUINC::lazyimgui::init() {
    ...
    al_set_window_title(this->__display, "lazyimgui window");
...
const bool GUINC::lazyimgui::running() {
    return __running;
}

ImDrawList* GUINC::lazyimgui::draw() {
    if(!__init)         throw std::runtime_error("ERROR: newFrame called when library hasn't been initialised.");
